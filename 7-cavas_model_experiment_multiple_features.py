...
# =============================================================================
# STEP 3 — PuckTrick noise helpers (sequenziali, una feature alla volta)
# =============================================================================

def _make_feature_strategy(noise_type, affected, percentage):
    """Strategy per i metodi feature-level (missing/outliers/noise)."""
    base = {
...
def _make_labels_strategy(affected, percentage):
    """Strategy per il metodo "labels" (corruzione di una colonna target int)."""
    return {
        "selection_criteria": "all",
        "percentage": percentage,
        "mode": "new",
...
def apply_feature_noise_sequential(sdf, feature_cols, noise_type, percentage):
    """
    Sporca le *feature_cols* UNA ALLA VOLTA con il metodo *noise_type*.
    PuckTrick supporta una sola feature per chiamata: si sporca la prima,
    si prende il DataFrame risultante, si sporca la seconda sul risultato,
    e cosi' via. Restituisce il DataFrame finale sporco.
    """
    if noise_type not in ("missing", "outliers", "noise"):
        raise ValueError(
            f"apply_feature_noise_sequential supporta solo "
    ...
    for feat in feature_cols:
        ...
        print(f"  ↳ sporcata feature '{feat}' con {noise_type} al {percentage*100:.1f}%")
    return sdf


def apply_label_noise_sequential(sdf, target_cols, percentage):
    """
    Sporca le colonne target *target_cols* UNA ALLA VOLTA con il metodo
    "labels", agendo sulla rispettiva colonna ENCODED (int). Il DataFrame
    deve gia' essere stato passato attraverso l'encoding (Label_enc /
    label_generic_enc presenti). Restituisce il DataFrame finale.
    """
    for tcol in target_cols:
        if tcol not in TARGET_TO_ENCODED:
            raise ValueError(
        ...
        if '_pucktrick_row_id' in sdf.columns:
            sdf = sdf.drop('_pucktrick_row_id')
        print(f"  ↳ sporcata target '{tcol}' (encoded '{enc_col}') con labels "
              f"al {percentage*100:.1f}%")
    return sdf
...
def prepare_whole_dataset_from_scratch(columns_to_insert_noise, percentage, noise_type):
    ...
    # ─────────────────────────────────────────────────────────────────────
    # SPORCATURA SEQUENZIALE DEL GRUPPO (una colonna alla volta)
    #   1) feature non-target  -> noise_type, sulla colonna RAW (pre-encoding)
    #   2) encoding di train/val/test
    #   3) colonne target      -> "labels", sulla colonna ENCODED (post-encoding)
    # ─────────────────────────────────────────────────────────────────────
    ...
    if feature_to_dirty:
        dirty_train = apply_feature_noise_sequential(
    ...
    if target_to_dirty:
        ...
        dirty_train = apply_label_noise_sequential(
