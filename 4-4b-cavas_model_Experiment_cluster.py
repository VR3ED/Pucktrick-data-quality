    ...
    for seed in RANDOM_SEEDS:
            ...
            for metodo in PUCKTRICK_METHODS_MAIN:
                for pct in PERCENTAGES:
                    ...
                    if experiment_already_exists(
                        f'Experiment_{metodo}_{colonna_da_sporcare.replace("/", "_")}_{pct*100:.1f}'
                    ):
                        print(f"⏭️  Skipping existing experiment: {metodo} on {colonna_da_sporcare} at {pct*100:.1f}%")
                        continue
                    ...
                    try:
                        ...
                        print(f"[rs={seed}] Error: {metodo} on {colonna_da_sporcare} at {pct*100:.1f}%: {e}")
                    finally:
                        clear_memory()
        ...
        for metodo in PUCKTRICK_METHODS_DUP:
            for pct in PERCENTAGES:
                ...
                if experiment_already_exists(
                    f'Experiment_{metodo}_{colonna_dup.replace("/", "_")}_{pct*100:.1f}'
                ):
                    continue

                try:
                    run_single_experiment(colonna_dup, metodo, pct)
                except Exception as e:
                    print(f"[rs={seed}] Error: {metodo} on {colonna_dup} at {pct*100:.1f}%: {e}")
                finally:
                    clear_memory()
