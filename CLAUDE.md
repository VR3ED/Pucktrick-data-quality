...
| `6B-Experiment-evaluation.ipynb` | Evaluates **Experiment B** (`complete_experiments/Experiment_B`, complete dataset, 20 seeds, single- vs. multiple-feature scenarios + `labels_experiment`). |
| `build_eval_notebooks.py` | Generator: `python3 build_eval_notebooks.py` regenerates **both** 6A/6B notebooks from one shared template. Edit the template here, not the notebooks directly. |

Aggregated tables are written to `evaluation_results/Experiment_{A,B}/`: `aggregate_{mcc,f1,accuracy}.csv`, `baseline_metrics.csv`, and significance tests (`significance_mcc_binary_vs_baseline.csv`, `significance_mcc_binary_vs_lownoise.csv`) comparing noisy runs against the clean `Baselines/` and against the lowest noise level.
...
| `6A-/6B-Experiment-evaluation.ipynb` | Current evaluation notebooks (see **Evaluation** above); generated by `build_eval_notebooks.py` |
